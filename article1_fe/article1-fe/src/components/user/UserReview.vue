  ...
  <div class="review-page">
    ...
    <div class="review-table">
      <div class="table-header">
        <span class="header-cell">selectDate</span>
        <span class="header-cell">customDate</span>
      ...
      <div v-for="(item, index) in paginatedReviews" :key="item.reviewSeq" class="table-row">
        <div class="table-cell">{{ selectedRecords.find(user => user.selectSeq === item.selectSeq)?.selectDate.slice(0, 10) }}</div>
        <div class="table-cell">{{ selectedRecords.find(user => user.selectSeq === item.selectSeq)?.customDate.slice(0, 10) }}</div>
        ...
        <div class="table-cell review-content">
          ...
          <!-- 옷 정보 슬라이더 추가 -->
          <div v-if="item.outfits && item.outfits.length > 0" class="outfit-slider">
    ...
    const fetchDataSelectedRecords = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          console.error("아웃핏 이력 조회 실패", response.status);
        }
      } catch (error) {
