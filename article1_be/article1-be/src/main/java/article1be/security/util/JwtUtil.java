...
public class JwtUtil {
    ...
    public Authentication getAuthentication(String token) {
        // Token Claims 에서 Subject(sub) 추출하여 userSeq를 빼온다
        Claims claims = Jwts.parserBuilder()
                .setSigningKey(SECRET_KEY)
                .build()
                .parseClaimsJws(token)
                .getBody();
        String userSeq = claims.getSubject(); // userSeq는 JWT 토큰의 subject에 저장되어있음

        // userSeq에 해당하는 사용자 정보를 데이터베이스에서 가져온다. 이때, UserDetails 객체가 반환
        UserDetails userDetails = userService.loadUserByUsername(userSeq);

        // UsernamePasswordAuthenticationToken 객체를 생성하여 Authentication 객체(Spring Security가 인증 상태를 관리하는 데 사용)를 만든다.
